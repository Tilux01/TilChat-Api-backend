...
app.post("/voiceAi", (req,res) =>{
    ...
    const query = async(userPrompt) =>{
        ...
        try {
            if (userPrompt.toLocaleLowerCase().includes("sqi") || userPrompt.toLocaleLowerCase().includes("sql") || userPrompt.toLocaleLowerCase().includes("sqi college")) {
                userPrompt = "Do you know sqi college of ICT in Nigeria and what do they do."
            }
            const result = await ai.models.generateContent({
                model: "gemini-2.5-flash-lite",
                contents: userPrompt
